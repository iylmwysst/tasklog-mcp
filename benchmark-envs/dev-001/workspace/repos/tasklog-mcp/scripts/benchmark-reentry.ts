...
function buildOpenDiscoveryAnswerFromNormalized(
...
function buildOpenDiscoveryNormalizedPayload(
  activeWork: WorkRecord | undefined,
  openWorks: WorkRecord[],
  logs: SessionLogEntry[],
): string {
  ...
  return JSON.stringify({
    active_work: activeWork
      ...
      : null,
    open_work_preview: preview.map((work) => ({
      work_id: work.work_id,
      title: work.title,
      status: work.status,
      next_step_summary: normalizeOptional(latestNextStep(logs.filter((entry) => entry.work_id === work.work_id))),
      last_log_summary: normalizeOptional(latestLog(logs.filter((entry) => entry.work_id === work.work_id))?.summary),
    })),
  }, null, 2);
}
...
async function benchmarkOpenWorks(paths: LogbookPaths, limit: number): Promise<StrategyRun[]> {
  ...
  const jsonNormalizedScan = await timed(async () => {
    const text = buildOpenDiscoveryNormalizedPayload(activeWork, openWorks, logs);
    const actualAnswer = buildOpenDiscoveryAnswerFromNormalized(activeWork, openWorks, logs);
    return {
      text,
      checks,
      answerFields: compareOpenDiscoveryAnswers(expectedAnswer, actualAnswer),
  ...
  jsonNormalizedScan.strategy = "json_state_normalized_scan";
  tasklog.strategy = "tasklog_get_active_plus_list_works";

  return [noContinuity, notebook, jsonScan, jsonNormalizedScan, tasklog];
}
...
function buildWorkReentryAnswerFromNormalized(
  work: WorkRecord,
...
function buildWorkReentryNormalizedPayload(
  work: WorkRecord,
  logsForWork: SessionLogEntry[],
  artifactPaths: string[],
): string {
...
async function benchmarkWorkReentry(paths: LogbookPaths, work: WorkRecord): Promise<StrategyRun[]> {
  ...
  const jsonNormalizedScan = await timed(async () => {
    const text = buildWorkReentryNormalizedPayload(work, logsForWork, artifactPaths);
    const actualAnswer = buildWorkReentryAnswerFromNormalized(work, logsForWork, artifactPaths);
    return {
      text,
      checks,
      answerFields: compareWorkReentryAnswers(expectedAnswer, actualAnswer),
  ...
  const tasklog = await timed(async () => withActiveContextRestored(paths, async () => {
    const [activeContext, workContext] = await Promise.all([
      ...
      readWorkContext(paths, work.work_id, { surface: "brief" }),
    ]);
    ...
    const actualAnswer: WorkReentryAnswer = {
      ...
      scope_paths: workContext.reentry_brief?.scope_paths ?? workContext.work.scope_paths,
      latest_log_summary: normalizeOptional(
        workContext.reentry_brief?.latest_log_summary ?? workContext.recent_logs.at(0)?.summary ?? latestLog(logsForWork)?.summary,
      ),
      next_step_summary: normalizeOptional(workContext.reentry_brief?.next_step_summary ?? workContext.next_step_summary),
      artifact_files: workContext.reentry_brief?.artifact_files ?? ARTIFACT_FILES.filter((fileName) => {
...
function summarizeScenario(title: string, prompt: string, runs: StrategyRun[]) {
  ...
  const rawStateRun = runs.find((run) => run.strategy === "json_state_scan");
  const normalizedStateRun = runs.find((run) => run.strategy === "json_state_normalized_scan");
  return {
    title,
    prompt,
    runs: runs.map((run) => ({
    ...
    tasklog_vs_markdown_reduction_percent: notebookRun && tasklogRun
      ...
      : null,
    tasklog_vs_raw_state_reduction_percent: rawStateRun && tasklogRun
      ? scenarioReductionPercent(rawStateRun, tasklogRun)
      : null,
    tasklog_vs_normalized_state_reduction_percent: normalizedStateRun && tasklogRun
      ? scenarioReductionPercent(normalizedStateRun, tasklogRun)
      : null,
    tasklog_vs_json_reduction_percent: rawStateRun && tasklogRun
      ? scenarioReductionPercent(rawStateRun, tasklogRun)
      : null,
  };
}
...
function printScenario(title: string, prompt: string, runs: StrategyRun[]): void {
  ...
  if (noContinuityRun && tasklogRun) {
    ...
    if (rawStateRun) {
      console.log(`- ${describeReduction(rawStateRun, tasklogRun)}`);
    }
    if (normalizedStateRun) {
      console.log(`- ${describeReduction(normalizedStateRun, tasklogRun)}`);
    }
...
function includeOpenDiscovery(options: CliOptions, manifest?: BenchmarkManifest): boolean {
...
function manifestWorkIds(manifest: BenchmarkManifest, split?: CliOptions["split"]): string[] {
...
async function main(): Promise<void> {
  ...
  const shouldIncludeOpenDiscovery = includeOpenDiscovery(options, manifest);
  ...
  if (options.json) {
    console.log(JSON.stringify({
      ...
      open_work_discovery_included: shouldIncludeOpenDiscovery,
      notes: [
  ...
  console.log(`- open_work_discovery_included: ${shouldIncludeOpenDiscovery}`);
