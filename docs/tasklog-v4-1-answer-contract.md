# Tasklog V4.1 Answer Contract

This document freezes the answer contract for the SWE-grounded `V4.1` go/no-go round.
...
`V4.1` keeps the same output shape as `V4`.
The repair is not a schema rewrite.
It is a prompt and grading-boundary rewrite.
...
These exact labels are interpreted by:
...
- non-applicable string fields must be the literal empty string `""`
- do not use placeholders such as `n/a`, `none`, `unknown`, or explanatory prose in fields that should be empty
...
The strict contract still includes only required fields.
`selection_rationale` remains appendix-only.
