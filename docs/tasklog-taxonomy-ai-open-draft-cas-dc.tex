...
This ordering is deliberate. If the current work is unknown, there is no stable basis for downstream reasoning. If work is known but authority is unresolved, the system may act on the wrong governing truth. If both are fixed but readiness is unresolved, even a plausible step may be invalid now. Only after these conditions are satisfied does it make sense to ask about concrete action content. Table~\ref{tab:loss-classes} defines the loss classes operationally.

\begin{strip}
\captionsetup{type=table}
\captionof{table}{Loss classes.}
...
Many resumptive episodes contain several plausible problems at once. To keep the taxonomy usable, each episode receives one \emph{dominant loss class}: the first missing or invalid object whose resolution is necessary before a safe next valid action can be produced. This rule does not deny that multiple pressures may coexist. Its purpose is to prevent classification from collapsing into an undisciplined list of everything relevant. Figure~\ref{fig:dominant-assignment} visualizes the dominant-assignment order that governs episode classification.

\begin{strip}
\centering
\captionsetup{type=figure}
\includegraphics[width=0.94\textwidth]{figures/fig1-beyond-memory-retrieval.pdf}
\caption{Dominant assignment in interrupted agentic work. Episodes are classified at the earliest unresolved governing layer: current work, authority, readiness, next valid action, and closure. Later uncertainty does not outrank an earlier unresolved layer.}
\label{fig:dominant-assignment}
\end{strip}
...
A secondary recovery-state layer shows how a continuity system may move through recovery after an interruption-boundary episode. Table~\ref{tab:recovery-states} lists the main states in this auxiliary layer.

\begin{strip}
\captionsetup{type=table}
\captionof{table}{Auxiliary recovery states.}
...
Future systems should represent \texttt{readiness} explicitly instead of assuming that successful continuity culminates in immediate action. The correct present outcome may be to ask, wait, escalate, or abstain. Systems that collapse all successful recovery into \texttt{act} will continue to mishandle exactly the cases where resumed action is most dangerous. Non-action outcomes must become first-class outputs, not failures of helpfulness. These include \texttt{waiting on approval}, \texttt{ask for one missing fact}, \texttt{revalidate stale state}, and \texttt{abstain pending authority\allowbreak\ resolution}.
