...
The `V4` SWE-grounded re-entry benchmark does need this contract because the frozen fixture pack is local but the evaluated answers come from external model runners.
...
Use `docs/benchmark-runner-metadata-sample.json` as a generic starter artifact.
For the SWE-grounded `V4` round, use `docs/tasklog-v4-runner-metadata-sample.json`.
Validate real metadata files with:
