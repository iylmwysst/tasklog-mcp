...
function registerTools(
  ...
  server.registerTool(
    "read_reentry_brief",
    {
      title: "Read re-entry brief",
      description:
        "Return the default concise re-entry surface for one work. Use this as the first read after selecting a work, before expanding to full work context.",
      inputSchema: {
        work_id: z.string().default("").describe("Optional explicit work id. Defaults to the active work when unambiguous."),
      },
      outputSchema: {
        context: reentryBriefContextSchema(),
      },
    },
    observeToolCall(logger, "read_reentry_brief", async ({ work_id }) => {
      const context = await readWorkContext(paths, work_id || undefined, {
        surface: "brief",
      });
      return {
        content: [{ type: "text", text: formatReentryBriefResponse(context) }],
        structuredContent: {
          context: toPublicReentryBriefContext(context),
        },
      };
    }),
  );
  ...
  server.registerTool(
    "read_work_context",
    {
      ...
      description:
        "Return the full work context for one work including artifact paths, context mode, recent logs, and optional summary loading for consolidated closed work. Prefer read_reentry_brief as the default first read.",
      inputSchema: {
        work_id: z.string().default("").describe("Optional explicit work id. Defaults to the active work when unambiguous."),
        include_summary: z
          .boolean()
