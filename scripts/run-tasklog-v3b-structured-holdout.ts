...
const ROUND_ID = "V3b";
const SOURCE_PACK_ROUND_ID = "V3a";
const RUNNER_NAME = "tasklog-benchmark-runner";
const RUNNER_VERSION = "2026-03-29-v3b";
...
function parseArgs(argv: string[]): CliOptions {
  ...
  const resolvedOutDir = outDir ?? path.join(
    DEFAULT_RUNS_ROOT,
    `v3b-${modelSlot}-rep${repetitionIndex}-${new Date().toISOString().replace(/[:.]/g, "-")}`,
  );
...
async function runCodexOnce(prompt: string, modelId: string, reasoningSetting: string, extraConfigs: string[]): Promise<ModelAnswer> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "tasklog-v3b-holdout-"));
...
async function runCodex(prompt: string, modelId: string, reasoningSetting: string, extraConfigs: string[]): Promise<ModelAnswer> {
  try {
    ...
    console.error(`[v3b] transient runner failure, retrying once\n${firstMessage}`);
    try {
      return await runCodexOnce(prompt, modelId, reasoningSetting, extraConfigs);
    } catch (secondError) {
      const secondMessage = secondError instanceof Error ? secondError.stack ?? secondError.message : String(secondError);
      throw new Error(`V3b runner failed twice for the same prompt.\n\nfirst_attempt:\n${firstMessage}\n\nsecond_attempt:\n${secondMessage}`);
    }
  }
}
...
async function runClaude(prompt: string, modelId: string, reasoningSetting: string): Promise<ModelAnswer> {
  try {
    ...
    const firstMessage = firstError instanceof Error ? firstError.stack ?? firstError.message : String(firstError);
    console.error(`[v3b] transient claude runner failure, retrying once\n${firstMessage}`);
    try {
      return await runClaudeOnce(prompt, modelId, reasoningSetting);
    } catch (secondError) {
      const secondMessage = secondError instanceof Error ? secondError.stack ?? secondError.message : String(secondError);
      throw new Error(`V3b Claude runner failed twice for the same prompt.\n\nfirst_attempt:\n${firstMessage}\n\nsecond_attempt:\n${secondMessage}`);
    }
  }
}
...
function assertValidMetadata(options: CliOptions): void {
  if (RUNNER_NAME !== "tasklog-benchmark-runner" || RUNNER_VERSION !== "2026-03-29-v3b") {
    throw new Error("Runner identity drift detected. V3b must use tasklog-benchmark-runner / 2026-03-29-v3b.");
  }
  if (options.repetitionIndex < 1 || options.repetitionIndex > 3) {
    throw new Error("V3b repetition_index must be 1, 2, or 3.");
  }
}
...
async function main(): Promise<void> {
  ...
  if (packManifest.benchmark_type !== BENCHMARK_TYPE || packManifest.round_id !== SOURCE_PACK_ROUND_ID) {
  ...
  if (answerKeyManifest.status !== "answer_keys_frozen") {
    throw new Error("V3b runner requires annotation/frozen-answer-key-manifest.json with status answer_keys_frozen.");
  }
  if (answerKeyManifest.round_id !== SOURCE_PACK_ROUND_ID) {
    throw new Error(`Unexpected frozen answer-key round at ${path.join(options.fixturesRoot, ANSWER_KEY_MANIFEST_PATH)}`);
  }
  ...
  const runId = `tasklog-v3b-${options.modelSlot}-rep${options.repetitionIndex}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  try {
    for (const fixtureId of fixtureIds) {
      ...
      for (const armId of options.arms) {
        ...
        console.error(
          `[v3b] slot=${options.modelSlot} rep=${options.repetitionIndex} fixture=${fixtureId} arm=${arm.display_label} model=${options.modelId}`,
        );
