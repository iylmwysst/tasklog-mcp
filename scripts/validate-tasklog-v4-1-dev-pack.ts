...
interface PackManifest {
  ...
  human_audit_policy_manifest?: string;
  annotation_family_briefs_manifest?: string;
...
async function main(): Promise<void> {
  ...
  const allocationPath = path.join(DOCS_ROOT, "tasklog-v4-1-dev-family-allocation-table.json");
  ...
  assert(packManifest.fixture_count === 16, `Expected 16 fixtures, found ${packManifest.fixture_count}`);
  ...
  for (const packFixture of packManifest.fixtures) {
    ...
    assert(JSON.stringify(questions.questions[0]?.required_fields ?? []) === JSON.stringify(requiredFields), `Questions ${packFixture.fixture_id} required_fields drifted from the V4.1 answer contract`);
