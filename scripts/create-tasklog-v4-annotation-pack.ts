...
const ANNOTATION_MODE = "llm_assisted_cross_model";
...
function annotatorPrompt(roleId: string, lane: { provider: string; model_family: string; reasoning_setting: string }, peerDir: string, ownDir: string): string {
  return [
    ...
    "- the frozen answer contract",
    "- the frozen grader contract",
    ...
    "",
    "1. read the draft and the referenced fixture files",
...
function adjudicatorPrompt(): string {
  return [
    ...
    "- any benchmark run outputs",
    "- any model-roster or run-layer artifact",
    "- any unfrozen change to the contracts or ontology",
    "",
    "## Task",
    "",
    "For each disagreement:",
    "",
    "1. compare only the frozen artifacts and the two completed drafts",
    "2. resolve the disagreement without inventing new policy",
    "3. append one adjudication entry with the frozen required fields:",
    "   - `fixture_id`",
    "   - `field_name`",
    "   - `annotator_a_value`",
    "   - `annotator_b_value`",
    ...
    "## Resolution Rules",
    "",
    "- prefer the narrowest rule already licensed by the contracts",
    "- do not rewrite fixture meaning after the fact",
    "- do not broaden next-step slots just to absorb both answers",
    "- respect the clarifying-vs-abstaining rule from the adjudication guide",
    "- respect `candidate_work_id_allowed`",
    "",
    "## Postcondition",
    "",
    "- disagreements remain preserved in `disagreement-log.json`",
    "- resolutions are recorded in `adjudication-log.json`",
    "- no answer key is considered frozen until every disagreement is resolved",
    "",
  ].join("\n");
}
...
async function main(): Promise<void> {
  ...
  for (const packFixture of packManifest.fixtures) {
    ...
    const frozenConstraints = {
      ...
      human_audit_status: fixtureManifest.generation_metadata?.human_audit_status ?? packFixture.human_audit_status,
    };
  ...
  const roleAssignment = {
    ...
    notes: [
      "This file records the planned V4 annotation lanes before dual annotation starts.",
      "Freeze the role assignment before any answer-key draft is completed.",
      "If one operator occupies multiple roles, record the blind-boundary risk in the authorship freeze note.",
    ],
  };
  ...
  await fs.writeFile(
    ...
    [
      "# Tasklog V4 Annotation Pack",
      "",
      "This directory scaffolds the pre-run answer-key workflow for the V4 SWE-grounded pack.",
      "",
      "Next steps:",
      ...
      "5. Replace `frozen-answer-key-manifest.json` placeholders with the final frozen keys.",
      "",
    ].join("\n"),
    "utf8",
  );
