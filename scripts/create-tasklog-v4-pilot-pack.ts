...
const FIXTURE_TYPE = "tasklog_v4_swe_grounded_fixture";
const ROUND_ID = "V4";

type WorkStatus = "active" | "blocked" | "done" | "planned";
type WorkDocKind = "notes" | "plan" | "spec" | "design" | "summary";
type AuditStatus = "audited_pass" | "selected_for_audit_pending" | "not_selected_for_audit";
...
interface AuditSubsetPlan {
  review_status?: string;
  selected_fixtures: Array<{
    fixture_id: string;
    review_status?: string;
  }>;
  pilot_fixture_ids: string[];
}
...
function parseArgs(argv: string[]): CliOptions {
  ...
  let outRoot = path.join(DEFAULT_LAB_ROOT, "fixtures-v4");
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (current === "--lab-root") {
      ...
      index += 1;
      outRoot = path.join(labRoot, "fixtures-v4");
      continue;
    }
    if (current === "--out-root") {
...
async function materializeFixture(params: {
    ...
    questions: [
      {
        ...
        prompt_version: "v4-reentry-question-2026-03-30a",
        answer_contract_version: "v4-answer-2026-03-30a",
        answer_contract_path: path.join(DOCS_ROOT, "tasklog-v4-answer-contract.md"),
  ...
  const fixtureJson = {
    ...
    evaluation_refs: {
      answer_contract_path: path.join(DOCS_ROOT, "tasklog-v4-answer-contract.md"),
      grader_contract_path: path.join(DOCS_ROOT, "tasklog-v4-grader-contract.md"),
  ...
  const fixtureStage = auditStatus === "audited_pass"
    ? "audited_subset_ready"
    : auditStatus === "selected_for_audit_pending"
      ? "selected_for_audit_pending"
      : "full_pack_seeded_pre_freeze";
  const manifest = {
    version: 1,
    round_id: ROUND_ID,
    fixture_id: allocation.fixture_id,
    ...
    seed_method: "v4_pack_builder_v1",
    seed_notes: scenario.seedNotes
  };
  ...
  await fs.writeFile(
    ...
    [
      ...
      "",
      auditStatus === "audited_pass" ? "Frozen audited-subset artifacts:" : "Frozen pre-freeze artifacts:",
...
async function main(): Promise<void> {
  ...
  const sourcePool = await loadJson<{ candidates: SourcePoolCandidate[] }>(path.join(DOCS_ROOT, "tasklog-v4-source-pool-manifest.json"));
  const allocationDoc = await loadJson<{ fixtures: AllocationFixture[] }>(path.join(DOCS_ROOT, "tasklog-v4-family-allocation-table.json"));
  const promptVersions = await loadJson<PromptVersions>(path.join(DOCS_ROOT, "tasklog-v4-prompt-versions.json"));
  const auditPlan = await loadJson<AuditSubsetPlan>(path.join(DOCS_ROOT, "tasklog-v4-audit-subset-plan.json"));
  ...
  const auditStatusByFixtureId = new Map(
    auditPlan.selected_fixtures.map((fixture) => [
      fixture.fixture_id,
      fixture.review_status === "audited_pass" ? "audited_pass" : "selected_for_audit_pending",
    ] as const),
  );
  ...
  for (const fixtureId of requestedFixtureIds) {
    ...
    await materializeFixture({
      allocation,
      auditStatus: auditStatusByFixtureId.get(fixtureId) ?? "not_selected_for_audit",
      fixtureRoot,
      promptVersions,
      repoCommit,
      scenario,
  ...
  const requestedSet = new Set(requestedFixtureIds);
  const isFullAllocation = requestedFixtureIds.length === allocationDoc.fixtures.length;
  const allRequestedAudited = requestedFixtureIds.every((fixtureId) => auditStatusByFixtureId.get(fixtureId) === "audited_pass");
  const packManifest = {
    version: 1,
    pack_id: isFullAllocation
      ? "tasklog-v4-full-allocation-pack"
      : allRequestedAudited
        ? "tasklog-v4-audited-subset-pack"
        : "tasklog-v4-custom-subset-pack",
    created_at: new Date().toISOString(),
    round_id: ROUND_ID,
    benchmark_type: BENCHMARK_TYPE,
    fixture_count: emittedFixtureIds.length,
    source_pool_manifest: path.join(DOCS_ROOT, "tasklog-v4-source-pool-manifest.json"),
    family_allocation_manifest: path.join(DOCS_ROOT, "tasklog-v4-family-allocation-table.json"),
    prompt_versions_manifest: path.join(DOCS_ROOT, "tasklog-v4-prompt-versions.json"),
    audit_subset_manifest: path.join(DOCS_ROOT, "tasklog-v4-audit-subset-plan.json"),
    answer_contract_manifest: path.join(DOCS_ROOT, "tasklog-v4-answer-contract.md"),
    grader_contract_manifest: path.join(DOCS_ROOT, "tasklog-v4-grader-contract.md"),
    ...
    fixtures: emittedFixtureIds.map((fixtureId) => {
      ...
      return {
        ...
        source_instance_id: allocation.source_instance_id,
        difficulty: allocation.difficulty,
        human_audit_status: auditStatusByFixtureId.get(fixtureId) ?? "not_selected_for_audit"
      };
    })
  };

  await writeJsonFile(path.join(options.outRoot, "pack-manifest.json"), packManifest);
  await writeJsonFile(path.join(options.outRoot, "pilot-pack-manifest.json"), packManifest);
